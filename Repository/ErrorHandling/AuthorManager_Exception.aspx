...
<body>
    ...
    <div>
		...
		<div style="background-color: #E0E0E0; height: 392px; width: 601px; padding: 10px">
			<asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:Pubs %>" DeleteCommand="DELETE FROM [authors] WHERE [au_id] = @au_id" InsertCommand="INSERT INTO [authors] ([au_id], [au_lname], [au_fname], [phone], [city], [state], [zip], [contract]) VALUES (@au_id, @au_lname, @au_fname, @phone, @city, @state, @zip, @contract)" SelectCommand="SELECT [au_id], [au_lname], [au_fname], [phone], [city], [state], [zip], [contract] FROM [authors] WHERE ([au_id] = @au_id)" UpdateCommand="UPDATE [author] SET [au_lname] = @au_lname, [au_fname] = @au_fname, [phone] = @phone, [city] = @city, [state] = @state, [zip] = @zip, [contract] = @contract WHERE [au_id] = @au_id" OnDeleted="SqlDataSource2_Deleted" OnInserted="SqlDataSource2_Inserted" OnUpdated="SqlDataSource2_Updated">
			<DeleteParameters>
